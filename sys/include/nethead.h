...
/** @brief Maximum count of neighbors tracked */
#define NETHEAD_NBR_CAPACITY  (3)
...
/**
 *  @brief Managed statistics on a 6LoWPAN L2.5 layer.
 */
typedef struct {
    uint8_t nbr_id;
    uint8_t l2addr[8];
...
/**
 * @brief   Reports on the 6LoWPAN link layer for a received packet.
 * 
 * @param[in] l2hdr Link layer header, with LQI
 * @param[in] hdrsize Size of header, max exceed size of hdr struct
 */
